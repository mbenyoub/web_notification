...
openerp.im = function(instance) {
    ...
    instance.im.ConversationManager = instance.web.Controller.extend({
                    ...
                    self.ensure_users(user_ids).then(function() {
                        _.each(messages, function(mes) {
                            if (mes.from_id != user_id.id){
                                var user = self.get_user(mes.from_id);
                                self.received_message(mes, user, user);
                            } else {
                                var user = self.get_user(mes.to_id);
                                var from_user = self.get_user(user_id.id);
                                self.received_message(mes, user, from_user);
                            }
                        });
                    });
        ...
        ensure_users: function(user_ids) {
            ...
            if (_.size(no_cache) === 0)
                ...
                return new instance.web.Model("im.user").call("read", [_.values(no_cache), ["name", "user", "im_status"]],
        ...
        received_message: function(message, user, from_user) {
            if (! this.get("window_focus")) {
            ...
            conv.received_message(message, from_user);
        },
    ...
    instance.im.Conversation = instance.web.Widget.extend({
        ...
        received_message: function(message, from_user) {
            if (this.shown) {
                this.set("pending", 0);
            } else {
                this.set("pending", this.get("pending") + 1);
            }
            this._add_bubble(from_user, message.message, message.date);
        },
        send_message: function(e) {
            ...
            var tries = 0;
            send_it().then(_.bind(function() {}, this), function(error, e) {
                e.preventDefault();
                tries += 1;
                if (tries < 3)
                    return send_it();
        ...
        _add_bubble: function(user, item, date) {
            var items = [item];
            if (user === this.last_user && this.last_user) {
